		...
		stage("Build") {
			steps {
				sshagent(credentials: ['dev-ssh']) {
					sh ''' 
 					        ssh -o StrictHostKeyChecking=no -l ubuntu 54.175.78.149 [ -n  $("ps | grep java") ] && killall java || echo "App not running"
						...
						ssh -o StrictHostKeyChecking=no -l ubuntu 54.175.78.149 "/home/ubuntu/biotestmine/gradlew cargoRunLocal > /dev/null 2>&1 &"
