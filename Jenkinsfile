		...
		stage("Build") {
			steps {
				sshagent(credentials: ['dev-ssh']) {
					sh ''' 
					        ssh -o StrictHostKeyChecking=no -l ubuntu 3.82.212.2 "if [[ -n 'ps aux | grep cargoRunLocal' ]]; then killall java; fi"
						...
						#ssh -o StrictHostKeyChecking=no -l ubuntu 3.82.212.2 "if [[ -n 'pgrep cargoRunLocal' ]]; then killall java; fi"
