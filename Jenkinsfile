		...
		stage("Build") {
			steps {
				sshagent(credentials: ['dev-ssh']) {
					sh ''' 
					       # ssh -o StrictHostKeyChecking=no -l ubuntu 52.90.174.103 "pkill biotestmineApp"
					       # ssh -o StrictHostKeyChecking=no -l ubuntu 52.90.174.103 "ps -ef | grep java | awk '{print $2}' | xargs kill"*/
