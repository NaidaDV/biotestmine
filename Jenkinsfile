	...
	stages {
		stage("Build") {
			steps {
				sshagent(credentials: ['dev-ssh']) {
					sh ''' 
 					        #ssh -o StrictHostKeyChecking=no -l ubuntu $DEV_IP_JEN [ -n  $("ps | grep java") ] && killall java || echo "App not running"
		...
		stage("Test") {
			steps {
				sshagent(credentials: ['dev-ssh']) { 
					sh '''
					ssh -o StrictHostKeyChecking=no -l ubuntu $DEV_IP_JEN "export $(cat ./biotestmine/)"	
